...
Each row represents a single flight
...
A negative departure delay means the flight departed earlier than its scheduled departure time.
...
The code filters for flights going to either RDU or GSO and then keeps only those flights where either the arrival delay or departure delay is negative, meaning the flight either departed early or arrived early.
...
flights |> summarize(mean_arr_delay = mean(arr_delay, na.rm = TRUE))
```
